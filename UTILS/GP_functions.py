    ...
    def __init__(self, train_x, train_y, likelihood, num_classes, kernal):
        ...
        if kernal == "matern":
            self.covar_module = ScaleKernel(
                MaternKernel(nu=0.5, batch_shape=torch.Size((num_classes,))),
    ...
    def predict(self, input):
        ...
        with gpytorch.settings.fast_pred_var(), torch.no_grad():
            ...
            observed_pred = self.likelihood(
                self.model(input)
            )  # likelihood predictions mean and var
...
def save_results(trainX, trainy, testX, testy, kernal, n_iterations=300, n_samples=100):
    ...
    likelihood = DirichletClassificationLikelihood(
        trainy.long(), learn_additional_noise=True
    )
    ...
    train_dist, train_observed_pred, train_pred_means, train_pred = trainer.predict(
        trainX
    )
    ...
    train_probabilities = (
        train_pred_samples / train_pred_samples.sum(-2, keepdim=True)
    ).mean(0)
    ...
    test_probabilities = (
        test_pred_samples / test_pred_samples.sum(-2, keepdim=True)
    ).mean(0)
    test_prob_stds = (test_pred_samples / test_pred_samples.sum(-2, keepdim=True)).std(
        0
    )
