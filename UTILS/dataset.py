...
def remove_duplicates(datapath=None, filename=None, df=None):
    """remove duplicates basedon 'base_rdkit_smiles col
    provide either (datapath+filename.csv) OR df to check
    returns df_cleaned: cleaned df with no duplicates"""
...
def create_folds(df, num):
    """Stratified kfold splitter to ensure constant inactive:active ratio in all folds
    df: df to split (5 is default)
    num (int): random seed
    returns: 
    df: with fold column indicating which fold# that row is"""
...
def over_sampling(
    data_path=None, filename=None, df=None, sampling=None, printOut=False
):
    """Oversample the (already scaled) dataset using SMOTE or ADASYN
    Keeps the feature names and id cols
    if providing filename, provide path (data_path) and filename (filename.csv) to read
    if providing a df, provide df
    sampling (str): 'SMOTE' or 'ADASYN'
    returns: oversampled dataframe, synthetic labels for synthetic samples
    """
    ...
    trainX_temp, trainy_temp = oversample.fit_resample(
        trainX.to_numpy(), trainy.to_numpy().reshape(-1)
    )
    if printOut:
    ...
    train_resamp = pd.concat(
        [combined_ids, trainX_resamp, trainy_resamp[["active"]]], axis=1
    )

    print(train_resamp.columns[train_resamp.columns.duplicated()])
    test_df_final = pd.concat(
        [
            test_just_ids.reset_index(drop=True),
            testX.reset_index(drop=True),
            testy.reset_index(drop=True),
        ],
        axis=1,
    )
...
def under_sampling(data_path=None, filename=None, df=None):
    ...
    trainX_temp, trainy_temp = undersample.fit_resample(
        trainX.to_numpy(), trainy.to_numpy().reshape(-1)
    )
...
def featurize(feat_type,data_path=None,filename=None,
    moe_path=None,moe_file=None,moe_df=None,df=None,
    mfp_radius=2,nBits=2048):
    """choose featurization strategy adn return featurized df
    feat_type (str): MOE or MFP for Molecular Operating Environment or 
    Morgan Fingerprint Features 
    if providing file, provide data_path+filename.csv to featurize 
    if using MOE, provide moe_path and moe_file.csv
    OR moe_df 
    if using df to featurize, provide df"""
...
def create_moe(
    data_path=None, filename=None, moe_path=None, moe_file=None, df=None, moe_df=None
):
    ...
    final_df = moe_df.merge(
        df, how="outer", on=["base_rdkit_smiles"], suffixes=("_moe_desc", "_og")
    )
...
def smiles_to_fps(smiles_list, radius=2, nBits=2048):
    """Given a lsit of smiles strings (intended to be normalized, pka normalized
    return circular morgan fingerprint features 2048 bit vectors as features"""
    fps = []
